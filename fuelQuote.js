...
function checkFormFG(galsRequested, delDATE, suggestedPrice, totalAMT)
{
   if (!galsRequested) //makes sure that you inputted something in the name box
   {
      return false;
   }

   if (!delDATE) //checks to make sure the password is withing the length restrictions
   {
      return false;
   }

   if (!suggestedPrice) //checks to make sure that the password has lower case letters
   {
      return false;
   }

   if (!totalAMT) //checks to make sure the password has uppercase letters
   {
      return false;
   }
}