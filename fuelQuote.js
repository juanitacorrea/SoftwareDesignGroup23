...
{
    isInQuotes,
    calcAmountWithMargin,
    calcSuggPrice,
    calcCoeffWithoutGal
 };
 
 
  function isInQuotes(username, database, tester)   // to check the login 
  {
      //copyOfObjPeople = JSON.parse(database);
      copyOfObjPeople = database;
 ...
 function calcSuggPrice(currentPrice, locationFactor, rateHistory, gallonsRegFactor, companyProfitFactor)
 {
     console.log("The Current Price", currentPrice)
     console.log("The Location", locationFactor)
     console.log("The History", rateHistory)
     console.log("The Reg Factor", gallonsRegFactor)
     console.log("The Company", companyProfitFactor)
    var margin = currentPrice * (locationFactor - rateHistory + gallonsRegFactor + companyProfitFactor);
    console.log("The margin", margin)
    var suggPrice = currentPrice + margin;
    console.log("The Sugg Price", suggPrice)
 
 
    return suggPrice;
 }
 
 function calcCoeffWithoutGal(rateHistory, locationFactor, companyProfitFactor)
 {
    console.log(locationFactor + " - " + rateHistory + " + " + companyProfitFactor);
    var coeff = locationFactor - rateHistory + companyProfitFactor;
 
 
    return coeff;
 }
 
 function calcAmountWithMargin(gallons, suggPrice)
 {
    var totalAmountWMargin = gallons * suggPrice;
    console.log("total", totalAmountWMargin)
